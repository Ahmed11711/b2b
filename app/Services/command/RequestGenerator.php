...
class RequestGenerator
{
    public static function make(string $model)
    {
        try {
                ...
                foreach ($columnsFullInfo as $col => $info) {
                    ...
                    if (Str::endsWith($col, '_id')) {
                        ...
                        if (Schema::hasTable($relatedTable)) {
                            ...
                            // حقن حقل العرض ليتم استخراجه في BaseRequest
                            $rule[] = "display_field:{$displayCol}";
                        }
                    }

                    // 3. معالجة الملفات والصور
                    if (preg_match('/(image|img|file|attachment|photo|picture|logo|avatar)/i', $col)) {
            ...
            return "✅ [Request Success] Store and Update requests generated with Schema Support and Display Fields!";
        } catch (\Throwable $e) {
    ...
    private static function generateStub($className, $rules, $namespace)
    {
        ...
        foreach ($rules as $key => $rule) {
            ...
            foreach ($rulesArray as $singleRule) {
                // مصفوفة الرسائل (تم اختصارها لسهولة القراءة وتجنب تكرار الـ Logic)
                ...
                if (str_contains($singleRule, 'max:')) {
                    ...
                    $msg = str_contains($rule, 'file') ? "greater than {$maxVal} KB." : "greater than {$maxVal} characters.";
